...
export function RequireAuthSheet({visible,next,onClose}:{visible:boolean;next:string;onClose:()=>void}){
  ...
  return <Modal visible={visible} transparent animationType="slide" onRequestClose={()=>router.replace('/(auth)/login')}><View style={s.backdrop}/><View style={s.sheet}><View style={s.handle}/><View style={s.icon}><AppIcon name="shield-checkmark" size={28} color={BRAND.RED}/></View><Text style={s.title}>Connectez-vous pour continuer</Text><Text style={s.sub}>Votre compte protege vos commandes, adresses et conversations avec le support.</Text><Pressable style={s.primary} onPress={()=>router.replace('/(auth)/login')}><AppIcon name="logo-whatsapp" size={20} color="#fff"/><Text style={s.primaryText}>Continuer vers la connexion</Text></Pressable></View></Modal>
}
